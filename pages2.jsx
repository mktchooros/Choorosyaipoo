...
/* ====================================================================
   ...
   ==================================================================== */
function PageStock({ stock, go, openItem }) {
      ...
      {view === "table" ? (
                  ...
                  <tr key={p.sku} onClick={()=>openItem(p.sku)} style={{cursor:"pointer"}}>
                    ...
                    <td>
                      <div style={{display:"flex", gap:3, alignItems:"flex-end", height:24}}>
                        {dist.map(d => (
                          <div key={d.code} title={`${d.code}: ${d.qty}`} style={{
                            width: 14,
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(260px, 1fr))", gap:14}} className="stagger">
          {filtered.map(p => {
            ...
            return (
              <div key={p.sku} className="card" onClick={()=>openItem(p.sku)} style={{cursor:"pointer", padding:18, transition:"all 0.22s"}}>
...
/* ====================================================================
   ...
   ==================================================================== */
function PageItemDetail({ sku, stock, movs, back }) {
        ...
        <div className="card">
          ...
          <div className="card-pad" style={{display:"flex", flexDirection:"column", gap:14}}>
            {dist.map((d, i) => {
              ...
              return (
                <div key={d.code} style={{display:"grid", gridTemplateColumns:"30px 1fr auto", gap:12, alignItems:"center"}}>
...
/* ====================================================================
   Locations page
   ==================================================================== */
function PageLocations({ stock, movs }) {
  const [sel, setSel] = useState("WH-A");
  const cur = LOCATIONS.find(l => l.code === sel);
  ...
  const utilization = Math.min(100, (totalItems / cur.capacity) * 100);
  ...
  return (
    <div className="page">
      <div className="page-head">
        ...
        <div className="page-actions">
          <button className="btn"><Icon name="shuffle" size={15}/>ย้ายสินค้า</button>
          <button className="btn btn-accent"><Icon name="plus" size={15}/>เพิ่ม Location</button>
        </div>
      </div>
      ...
      <div className="loc-board mb-md" style={{marginBottom:18}}>
        {LOCATIONS.map(l => {
          ...
          return (
            <div key={l.code} className={`loc-card ${sel === l.code ? "selected" : ""}`} onClick={()=>setSel(l.code)}>
