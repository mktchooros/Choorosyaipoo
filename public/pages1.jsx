...
/* ====================================================================
   Receive (รับเข้า)
   ==================================================================== */
function PageReceive({ stock, setStock, addMov, recents, locations = [] }) {
            ...
            <div className="grid-3 mb-md">
              ...
              <div className="field">
                ...
                <select className="select" value={loc} onChange={e=>setLoc(e.target.value)}>
                  {locList.filter(l=>l.type!=="vehicle").map(l => <option key={l.code} value={l.code}>{l.name}</option>)}
                </select>
              </div>
            </div>
...
/* ====================================================================
   Issue (เบิก)
   ==================================================================== */
function PageIssue({ stock, setStock, addMov, recents, customers = [], locations = [] }) {
  const toast = useToast();
  const [purpose, setPurpose] = useState("ขายให้ลูกค้า");
  const [cust, setCust] = useState((customers[1] || {}).id || "");
  const [from, setFrom] = useState((locations[0] || {}).code || "WH-A");
  ...
  const submit = () => {
    ...
    for (const l of lines) {
      if (!l.qty || +l.qty <= 0) {
        toast.push({ kind:"danger", title:"จำนวนไม่ถูกต้อง", msg: `${PRODUCTS.find(p=>p.sku===l.sku).name} ต้องระบุจำนวน` });
        return;
      }
    ...
    if (purpose === "ขายให้ลูกค้า" && !cust) {
      toast.push({ kind:"danger", title:"กรุณาเลือกลูกค้า" });
      return;
    }
    if (purpose === "โอนภายใน" && !to) {
      toast.push({ kind:"danger", title:"กรุณาเลือกปลายทาง" });
      return;
    }

    const toName = purpose === "ขายให้ลูกค้า" ? (customers.find(c => c.id === cust) || {}).name : locList.find(l => l.code === to)?.name || to;
    ...
    setStock(prev => {
      ...
      lines.forEach(l => {
        ...
        // Increase at destination (if transfer)
        if (purpose === "โอนภายใน") {
          next[l.sku] = { ...next[l.sku], [to]: (next[l.sku]?.[to] || 0) + (+l.qty) };
        }
      });
      return next;
    });
    ...
    // Log movement
    lines.forEach(l => {
      const movType = purpose === "ขายให้ลูกค้า" ? "out" : purpose === "โอนภายใน" ? "transfer" : "out";
      addMov({
        type: movType,
        sku: l.sku,
        qty: +l.qty,
        from,
        to: purpose === "โอนภายใน" ? to : toName,
        ref,
        by: "User"
      });
    });

    const action = purpose === "ขายให้ลูกค้า" ? "เบิก" : purpose === "โอนภายใน" ? "ย้าย" : "เบิก";
    toast.push({ kind:"success", title:`บันทึก${action}เรียบร้อย`, msg: `${lines.length} รายการ · ${baht(totalAmt)} · ${toName}` });
            ...
            <div className="grid-3 mb-md">
              ...
              <div className="field">
                ...
                <select className="select" value={from} onChange={e=>setFrom(e.target.value)}>
                  {locList.map(l => <option key={l.code} value={l.code}>{l.name}</option>)}
                </select>
              </div>
              <div className="field">
                ...
                {purpose === "ขายให้ลูกค้า" ? (
                  ...
                  <select className="select" value={to} onChange={e=>setTo(e.target.value)}>
                    ...
                    {locList.filter(l => l.code !== from).map(l => <option key={l.code} value={l.code}>{l.name}</option>)}
                  </select>
                )}
              </div>
            </div>
